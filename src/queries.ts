...
export async function getUnlistDevices(gqlClient: GraphQLClient, owner: string, chainId = 84532) {
  const query: TypedDocumentNode<GetDevicesQuery> = parse(gql`
    query getDevices($chainId: Bigint, $owner: String, $limit: Int, $offset: String) {
      Device(filter: {owner: { eq: $owner}, product: {factory: {chain: {chainId: {eq: $chainId}}}}}, first: $limit, after: $offset) {
        address
        id
        owner
        product {
  ...
  const variables: GetDevicesQueryVariables = {
    chainId,
